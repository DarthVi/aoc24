export async function day1(path: string) {
  return { part1: await day1_part1(path), part2: await day1_part2(path) };
}
...
async function day1_part1(path: string) {
  ...
  return finalArr[0]
    .map((entry, index) => Math.abs(entry - finalArr[1][index]))
    .reduce((a, b) => a + b);
}
...
async function day1_part2(path: string) {
  ...
  return finalArr[0]
    .map(
      (entry, index) => entry * finalArr[1].filter((s) => s === entry).length,
    )
    .reduce((a, b) => a + b);
}
